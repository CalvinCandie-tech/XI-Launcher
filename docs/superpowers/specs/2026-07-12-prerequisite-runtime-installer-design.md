...
| .NET Framework 4.5.2 (offline, covers 4.0) | `https://download.microsoft.com/download/e/2/1/e21644b5-2df2-47c2-91bd-63c560427900/NDP452-KB2901907-x86-x64-AllOS-ENU.exe` | `6C2C589132E830A185C5F40F82042BEE3022E721A216680BD9B3995BA86F3781` | `/q /norestart` | 0, 3010 |
...
**Note on the .NET row:** the URL is confirmed directly from the official Microsoft Download Center page (`microsoft.com/en-us/download/details.aspx?id=42642`). No winget manifest exists for the bare 4.5.2 runtime (only for the Developer Pack, a different artifact), so there was no third-party-verified hash to cross-check against winget. Instead, the file was downloaded directly from that URL and hashed during planning (2026-07-12): 69,999,448 bytes, matching the ~66.8 MB Microsoft documents for this installer. Same trust model as the other four — Microsoft's own URL, hash pinned from the actual bytes — just derived directly instead of copied from a winget manifest. The .NET installer uses a different install technology (Windows component servicing, not MSI/Burn) and doesn't share the 1638 code — if a same-or-newer .NET is already present, the NDP setup typically just exits 0 quickly; no special-casing needed there.

All five URLs and hashes were also spot-checked during planning by actually downloading the VC++ 2012 x86 installer and computing its SHA256 live — it matched the pinned table value exactly, confirming the winget-sourced data is trustworthy.
