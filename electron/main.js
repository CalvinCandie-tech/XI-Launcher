...
async function initStore() {
  ...
  store = new Store({
    defaults: {
      ...
      xiloaderPath: '',
      profiles: [],
      activeProfile: 'Legendary',
      setupComplete: true,
      serverHost: '172.215.213.23',
      serverPort: '',
      loginUser: '',
      loginPass: '',
      lastLaunched: null,
...
// Locked-file message for addon uninstall. UI/HUD addons (XIUI, xivbar, XivParty…)
// load DLLs and map fonts/textures into the live Direct3D8 pipeline; Windows won't
// let those files be deleted while FFXI/Ashita holds them, so "run as admin" is the
// wrong advice — closing the game is the actual fix.
function gameRunningError(addonName, removed, locked) {
  const partial = (removed != null && locked != null)
    ? ` Removed ${removed} file(s); ${locked} are still in use.`
    : '';
  return `Couldn't fully remove ${addonName} — some of its files are in use.${partial} `
    + `This almost always means FINAL FANTASY XI / Ashita is still running (the game `
    + `locks an addon's DLLs, fonts and textures while it's loaded). Fully close the `
    + `game and Ashita, then uninstall ${addonName} again.`;
}
...
function buildDefaultProfileIni(name, xiloaderExe, serverHost, ffxiPath) {
  const ffxi = (ffxiPath || 'C:\\Program Files (x86)\\PlayOnline\\SquareEnix\\FINAL FANTASY XI').replace(/\//g, '\\');
...
[ashita.boot]
file         = ${xiloaderExe}
command      = --server ${serverHost || '172.215.213.23'}
...
async function autoGenerateDefaultProfile() {
  try {
    const activeProfile = store.get('activeProfile');
    if (!activeProfile) return;
    const ashitaPath = store.get('ashitaPath') || defaultAshitaPath;
    const profilePath = path.join(ashitaPath, 'config', 'boot', activeProfile + '.ini');
    if (fs.existsSync(profilePath)) return;
    const serverHost = store.get('serverHost') || '172.215.213.23';
    const ffxiPath = store.get('ffxiPath') || 'C:\\Program Files (x86)\\PlayOnline\\SquareEnix\\FINAL FANTASY XI';
    const xiloaderExe = defaultXiloaderPath.replace(/\//g, '\\') + '\\xiloader.exe';
    store.set('xiloaderPath', defaultXiloaderPath);
    const ini = buildDefaultProfileIni(activeProfile, xiloaderExe, serverHost, ffxiPath);
    fs.mkdirSync(path.dirname(profilePath), { recursive: true });
    fs.writeFileSync(profilePath, ini, 'utf8');
  } catch (e) {
    console.error('Failed to auto-generate default profile:', e);
  }
}
...
app.whenReady().then(async () => {
  ...
  await autoGenerateDefaultProfile();
  registerCSP();
  createWindow();
  createTray();
...
function registerIPC() {
  ...
  ipcMain.handle('import-custom-addon', async () => {
    ...
    // Picked a folder that's already inside addons/ — it's installed already; don't
    // copy it onto itself (which would recurse/error). Just report it.
    if (path.resolve(dest) === path.resolve(selected)) {
  ...
  // Restore a built-in addon/plugin from the launcher's bundled Ashita into the
  // user's configured install. Used by the Missing Addons/Plugins dialog "Fix".
  ipcMain.handle('restore-bundled-item', async (_, type, name) => {
    try {
      const ashitaPath = store?.get('ashitaPath');
      if (!ashitaPath) return { success: false, error: 'Ashita path not configured.' };
      const safe = sanitizeName(name);
      if (!safe) return { success: false, error: 'Invalid name.' };

      if (type === 'plugin') {
        const src = path.join(defaultAshitaPath, 'plugins', `${safe}.dll`);
        const dest = path.join(ashitaPath, 'plugins', `${safe}.dll`);
        if (path.resolve(src) === path.resolve(dest)) {
          return fs.existsSync(dest) ? { success: true, message: 'Already present' } : { success: false, error: 'Not available in the bundled Ashita.' };
        }
        if (!fs.existsSync(src)) return { success: false, error: `${safe}.dll is not in the bundled Ashita — update Ashita to restore it.` };
        fs.mkdirSync(path.dirname(dest), { recursive: true });
        fs.copyFileSync(src, dest);
        return { success: true, message: `Restored ${safe}.dll` };
      }

      const src = path.join(defaultAshitaPath, 'addons', safe);
      const dest = path.join(ashitaPath, 'addons', safe);
      if (path.resolve(src) === path.resolve(dest)) {
        return fs.existsSync(dest) ? { success: true, message: 'Already present' } : { success: false, error: 'Not available in the bundled Ashita.' };
      }
      if (!fs.existsSync(src)) return { success: false, error: `${safe} is not in the bundled Ashita — update Ashita to restore it.` };
      copyRecursive(src, dest);
      return { success: true, message: `Restored ${safe}` };
    } catch (e) {
      return { success: false, error: friendlyError(e, 'Restoring from bundled Ashita') };
    }
  });
  ...
  function getReshadeDllDir() {
    const xiloaderPath = store?.get('xiloaderPath');
    if (xiloaderPath && fs.existsSync(xiloaderPath)) return xiloaderPath;
    const ashitaPath = store?.get('ashitaPath');
    if (ashitaPath) {
      const bootDir = path.join(ashitaPath, 'bootloader');
      if (fs.existsSync(bootDir)) return bootDir;
    }
    return null;
  }
  ...
  ipcMain.handle('check-reshade', async (_, ffxiPath) => {
    try {
      ...
      const dllPath = path.join(dllDir, 'dxgi.dll');
      const disabledPath = path.join(dllDir, 'dxgi.dll.disabled');
      const shadersPath = path.join(ffxiPath, 'reshade-shaders');
      const iniPath = path.join(dllDir, 'ReShade.ini');
      // Check dgVoodoo in BOTH dllDir and ffxiPath
      const dgvReady = fs.existsSync(path.join(dllDir, 'D3D8.dll')) || fs.existsSync(path.join(ffxiPath, 'D3D8.dll'));
  ...
  ipcMain.handle('install-reshade', async (_, ffxiPath) => {
    try {
      ...
      // DLL + INI go to the xiloader dir (where the game exe runs from)
      const dllDir = getReshadeDllDir() || ffxiPath;
      fs.copyFileSync(dll32, path.join(dllDir, 'dxgi.dll'));

      // Shaders go to FFXI dir (referenced via absolute paths in INI)
  ...
  ipcMain.handle('uninstall-addon', async (_, ashitaPath, addonName, isPlugin, ashitaRoot) => {
    try {
      if (ashitaRoot) {
        ...
        for (const filePath of manifest) {
          ...
          try {
            ...
            // A loaded DLL / mapped font/texture can't be unlinked while FFXI is
            // running. Count it so we can warn instead of reporting a false success.
            if (e.code === 'EPERM' || e.code === 'EBUSY' || e.code === 'EACCES') locked++;
            console.error('[uninstall-addon] file:', e.message);
          }
        ...
        // If files were locked, keep the manifest so a later retry (after the game
        // is closed) can finish the job — and tell the user what to do.
        if (locked > 0) {
          return { success: false, error: gameRunningError(addonName, removed, locked) };
        }
      ...
      try {
        // maxRetries/retryDelay rides out transient AV/indexer locks. A DLL the
        // running client has LoadLibrary'd stays locked for the process lifetime,
        // so after retries exhaust we surface an actionable "close the game" error
        // rather than the generic "run as administrator" (admin can't unlink a
        // loaded module either).
        fs.rmSync(addonDir, { recursive: true, force: true, maxRetries: 5, retryDelay: 150 });
      } catch (e) {
        if (e.code === 'EPERM' || e.code === 'EBUSY' || e.code === 'ENOTEMPTY' || e.code === 'EACCES') {
          return { success: false, error: gameRunningError(addonName) };
        }
        throw e;
      }
      // Clean up stored SHA
